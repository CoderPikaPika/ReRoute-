    ...
    <div className="min-h-screen bg-[#f4f9fd] text-slate-900 lg:grid lg:grid-cols-[220px_1fr]">
      <aside className="relative overflow-hidden border-b border-[#d9e8f2] bg-[linear-gradient(180deg,#fbfdff_0%,#f5fbff_67%,#d6ecfa_100%)] text-[#1c385d] lg:min-h-screen lg:border-b-0 lg:border-r">
        <div className="flex h-full flex-col px-4 py-5">
          <NavLink
            className="flex items-center gap-2 px-3 text-xl font-extrabold tracking-tight text-[#103b76]"
            to="/dashboard"
          >
            <img
              alt="SeaNexus"
              className="h-10 w-10 object-contain mix-blend-multiply"
              src="/seanexus-logo.jpeg"
            />
          ...
          <div className="relative mt-auto hidden min-h-48 overflow-hidden pt-5 lg:block">
            ...
            <p className="relative mt-20 px-3 text-[11px] leading-5 text-[#65819a]">
              Cleaner seas
              <br />
      ...
      <div className="min-w-0">
        <header className="flex items-center border-b border-[#dbeaf3] bg-white px-5 py-3 shadow-sm sm:px-6">
          {/* Mobile */}
          <div className="sm:hidden">
            <p className="text-sm font-semibold text-[#0d3b75]">SeaNexus</p>
            <p className="text-sm text-slate-700">{user?.name}</p>
          </div>

          {/* User section */}
          <div className="flex flex-1 items-center gap-3">
            <span className="hidden text-xl text-[#254a73] sm:block">♧</span>

            <span className="hidden h-9 w-px bg-[#e1edf4] sm:block" />

            <span className="hidden h-9 w-9 place-items-center rounded-full bg-[#143e71] text-xs font-bold text-white sm:grid">
              {user?.name
                ?.split(' ')
                .map((part) => part[0])
                .slice(0, 2)
            ...
            <span className="hidden leading-tight sm:block">
              <b className="block text-xs text-[#1e3b61]">{user?.name || 'SeaNexus User'}</b>
              <small className="text-[10px] text-slate-500">Logistics Manager</small>
            </span>

            {/* Logout pushed to extreme right */}
            <button
              className="ml-auto rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-sky-300 hover:bg-sky-50 hover:text-sky-800"
